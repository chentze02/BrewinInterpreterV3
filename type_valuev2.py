...
def create_default_value(type_def):
    if isinstance(type_def, str):
        if type_def == InterpreterBase.BOOL_DEF:
            return Value(Type(InterpreterBase.BOOL_DEF), False)
        elif type_def == InterpreterBase.STRING_DEF:
            return Value(Type(InterpreterBase.STRING_DEF), "")
        elif type_def == InterpreterBase.INT_DEF:
            return Value(Type(InterpreterBase.INT_DEF), 0)
        elif type_def == InterpreterBase.NOTHING_DEF:
            return Value(Type(InterpreterBase.NOTHING_DEF), None)
        else:
            return Value(
                Type(type_def), None
            )  # the type is a class type, so we return null for default val, with proper class typ
...
class TypeManager:
    ...
    def add_tclass_type(self, tclass):
        tclass_type = Type(tclass, None)
        self.map_tclass_to_type[tclass] = tclass_type
    ...
    def check_type_compatibility(self, typea, typeb, for_assignment):
        ...
        # return false
        if "@" in typea.type_name and typeb.type_name == InterpreterBase.NULL_DEF:
            return True

        if "@" in typeb.type_name and typea.type_name == InterpreterBase.NULL_DEF:
            return True

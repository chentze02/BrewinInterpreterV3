...
class ObjectDef:
    ...
    def __init__(self, interpreter, class_def, anchor_object=None, trace_output=False):
        ...
        # construct default values for superclass fields all the way to the base class
        self.__init_superclass_if_any()

    # COME BACK HERE IF CODE BREAKS
    ...
    def call_method(self, method_name, actual_params, super_only, line_num_of_caller):
        ...
        obj_to_call_on = self.__get_obj_with_method(
            anchor, method_name, actual_params)
    ...
    def __execute_statement(self, env, return_type, code, exception=None):
        ...
        if tok == InterpreterBase.BEGIN_DEF:
            ...
            return self.__execute_print(env, code, exception)
    ...
    def __execute_try(self, env, return_type, code):
        ...
        status, return_value = self.__execute_statement(
            env, return_type, try_block)

        if status == ObjectDef.STATUS_THROW:
            env.block_nest()
            exception = self.interpreter.get_exception()
            env.create_new_symbol(InterpreterBase.EXCEPTION_VARIABLE_DEF)
            env.set(InterpreterBase.EXCEPTION_VARIABLE_DEF, exception)
    ...
    def __add_locals_to_env(self, env, var_defs, line_number):
        for var_def in var_defs:
            ...
            var_name = var_def[1]
            if "@" in var_def[0]:
                types_string = var_def[0]
                types = types_string.split('@')
                template_name = types[0]
                parametrized_types_name = types[1:]
                tclass_def = self.interpreter.tclass_index[template_name]
                num_of_expected_type = len(tclass_def[2])
                if len(parametrized_types_name) != num_of_expected_type:
                    self.interpreter.error(
                        ErrorType.TYPE_ERROR,
                        "wrong number of types for template instantiation",
                        line_number,
                    )
                # self.interpreter.instantiate_templated_class(
                #     template_name, var_def[0].line_num, parametrized_types_name, var_def[1])
            if len(var_def) == 2:
                # if there's no assigned value, then create a default value of the correct type
    ...
    def __execute_call(self, env, code):
        ...
        return ObjectDef.STATUS_PROCEED, return_val
    ...
    def __execute_throw(self, env, return_type, code):
        status, exception = self.__evaluate_expression(
            env, code[1], code[0].line_num)

        if exception.type().type_name != InterpreterBase.STRING_DEF:
            self.interpreter.error(
                ErrorType.TYPE_ERROR,
                "throw statement requires a string as an argument",
                code[0].line_num,
            )
        self.interpreter.create_exception(exception)
        if status == ObjectDef.STATUS_THROW:
            return status, exception
        return ObjectDef.STATUS_THROW, exception
    ...
    def __execute_return(self, env, return_type, code):
        if len(code) == 1:
            ...
            status, result = self.__evaluate_expression(env, code[1], 0)
            # CAREY FIX
            if status == ObjectDef.STATUS_THROW:
    ...
    def __propagate_type_to_null(self, var_def):
        if isinstance(var_def.value, tuple):
            var_def.value = var_def.value[1]
    ...
    def __evaluate_expression(self, env, expr, line_num_of_statement, exception=None):
        if type(expr) is not list:
            # locals shadow member variables
            var_def = env.get(expr)
            if var_def is not None:
                if expr == InterpreterBase.EXCEPTION_VARIABLE_DEF:
                    return ObjectDef.STATUS_PROCEED, var_def
            ...
            if expr == InterpreterBase.ME_DEF:
                return (
                    ObjectDef.STATUS_PROCEED, self.get_me_as_value()
        ...
        if operator in self.binary_op_list:
            ...
            if status == ObjectDef.STATUS_THROW:
                return status, "exception thrown"
        ...
        if operator in self.unary_op_list:
            ...
            if operand.type() == ObjectDef.BOOL_TYPE_CONST:
                ...
                return ObjectDef.STATUS_PROCEED, self.unary_ops[InterpreterBase.BOOL_DEF][operator](operand)

        # handle call expression: (call objref methodname p1 p2 p3)
        if operator == InterpreterBase.CALL_DEF:
            status, value = self.__execute_call_aux(
    ...
    def __execute_new_aux(self, env, code, line_num_of_statement):
        class_name = code[1]
        PRIMITIVE_TYPES = {InterpreterBase.INT_DEF, InterpreterBase.BOOL_DEF,
                           InterpreterBase.STRING_DEF, InterpreterBase.NOTHING_DEF}

        if "@" in class_name:
            template_class_content = class_name.split("@")
            template_class_name = template_class_content[0]
            template_class_parametrized = template_class_content[1:]
            if template_class_name not in self.interpreter.tclass_index:
                self.interpreter.error(
                    ErrorType.NAME_ERROR,
                    "invalid class name " + template_class_name,
                    line_num_of_statement,
                )
            for template_args in template_class_parametrized:
                if template_args not in self.interpreter.class_index and not PRIMITIVE_TYPES:
                    self.interpreter.error(
                        ErrorType.NAME_ERROR,
                        "invalid class name " + template_args,
                        line_num_of_statement,
                    )

            obj = self.interpreter.instantiate_templated_class(
                template_class_name, line_num_of_statement, template_class_parametrized, class_name)
        else:
    ...
    def __execute_call_aux(self, env, code, line_num_of_statement):
        ...
        actual_args = []
        for expr in code[3:]:
            status, valToAppend = self.__evaluate_expression(
                env, expr, line_num_of_statement)
            if status == ObjectDef.STATUS_THROW:
                return status, valToAppend
            actual_args.append(
                valToAppend
            )
        if not isinstance(obj, ObjectDef):
            self.interpreter.error(
                ErrorType.NAME_ERROR,
                "invalid object reference " + obj_name,
                line_num_of_statement,
            )
    ...
    def __set_field(self, field_name, value, line_num):
        ...
        self.__check_type_compatibility(
            var_def.type, value.type(), True, line_num)

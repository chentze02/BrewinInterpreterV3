...
class ClassDef:
    ...
    def __create_variable_def_from_field(self, field_def):
        if len(field_def) == 3:
            if "@" in field_def[1]:
                types_string = field_def[1]
                types = types_string.split('@')
                template_name = types[0]
                parametrized_types_name = types[1:]
                # instantiate_templated_class(
                #     self, class_name, line_num_of_statement, template_args, actual_full_class_name)
                self.interpreter.instantiate_templated_class(
                    template_name, field_def[0].line_num, parametrized_types_name, field_def[1])

            if field_def[1] not in PRIMITIVE_TYPES:
                var_def = VariableDef(
                    Type(field_def[1]), field_def[2], create_value(InterpreterBase.NULL_DEF))
            else:
                var_def = VariableDef(
        ...
        elif not self.interpreter.check_type_compatibility(
    ...
    def __create_method_list(self, class_body):
        ...
        for member in class_body:
            if member[0] == InterpreterBase.METHOD_DEF:
                ...
                #     #     self, class_name, line_num_of_statement, template_args, actual_full_class_name)
                #     self.interpreter.instantiate_templated_class(
                #         template_name, member[0].line_num, parametrized_types_name, member[1])
